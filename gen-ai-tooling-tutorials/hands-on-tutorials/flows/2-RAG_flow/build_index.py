...
def create_faiss_index(pdf_path: str) -> str:
    ...
    # Hash the filename to avoid long filename issue
    hashed_filename = hashlib.sha256(pdf_path.encode()).hexdigest()
    file_name = hashed_filename + f".index_{chunk_size}_{chunk_overlap}"
    ...
    # Ensure the directory for the index_persistent_path exists before acquiring the lock
    ...
    with acquire_lock(lock_path):
        if os.path.exists(os.path.join(index_persistent_path, "index.faiss")):
            ...
            if not os.path.exists(index_persistent_path):
                os.makedirs(index_persistent_path, exist_ok=True)
